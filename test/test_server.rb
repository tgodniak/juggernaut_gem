...
class TestServer < Test::Unit::TestCase
  ...
  class DirectClient
    ...
    def query_remove_channels_from_client(channels, client)
      self.transmit :command => :query, :type => :remove_channels_from_client, :client_id => client, :channels => channels
      self
    end
    def query_remove_channels_from_clients(channels, clients)
      self.transmit :command => :query, :type => :remove_channels_from_clients, :client_ids => clients, :channels => channels
      self
    end
    def query_add_channels_to_client(channels)
      self.transmit :command => :query, :type => :add_channels_to_client, :channels => channels
      self
    end
  ...
  context "Server" do
    ...
    context "remove channels request" do
      ...
      should "work on specific clients when requested" do
        with_server do
          ...
          self.new_client(:client_id => "pinocchio") { |c|
            c.subscribe %w(master slave1 slave2)
            c.query_remove_channels_from_clients %w(slave1 slave2), %w(homer pinocchio)
          }
        end
        assert_does_not_contain @connections.find { |c| c.instance_eval("@request[:client_id]") == "homer" }.channels, /slave/
        assert_does_not_contain @connections.find { |c| c.instance_eval("@request[:client_id]") == "pinocchio" }.channels, /slave/
        assert_contains @connections.find { |c| c.instance_eval("@request[:client_id]") == "marge" }.channels, /slave/
      end

      should "work on specific client when requested" do
        with_server do
          self.new_client(:client_id => "homer") { |c| c.subscribe %w(groupie master slave1 slave2) }
          self.new_client(:client_id => "marge") { |c| c.subscribe %w(master slave1 slave2) }
          self.new_client(:client_id => "pinocchio") { |c|
            c.subscribe %w(master slave1 slave2)
            c.query_remove_channels_from_client %w(slave1 slave2), "homer"
          }
        end
        assert_does_not_contain @connections.find { |c| c.instance_eval("@request[:client_id]") == "homer" }.channels, /slave/
        assert_contains @connections.find { |c| c.instance_eval("@request[:client_id]") == "marge" }.channels, /slave/
    ...
    context "add channels request" do

      should "work on specific client when requested" do
        with_server do
          self.new_client(:client_id => "homer") { |c| c.subscribe %w(groupie) }
          self.new_client(:client_id => "pinocchio") { |c|
            c.subscribe %w(groupie)
            c.query_add_channels_to_client %w(slave1 slave2)
          }
        end
        assert_contains @connections.find { |c| c.instance_eval("@request[:client_id]") == "pinocchio" }.channels, /slave/
        assert_does_not_contain @connections.find { |c| c.instance_eval("@request[:client_id]") == "homer" }.channels, /slave/
      end
